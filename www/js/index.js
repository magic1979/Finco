...
var app = {
    ...
    receivedEvent: function(id) {
		...
		if(PushbotsPlugin.isAndroid()){
			PushbotsPlugin.initializeAndroid("5656fe1217795908478b4572", "458447197779");
		}
...
function regToken() {
	...
	$.ajax({
		   type:"GET",
		   url:"http://interactivebusinessapp.it/device/set_token/PxgLiaL7dBgTYUzUyHZRNGIUlT5NIabyHrkZC57PHoJGiiAQZA/55qZrmzGcsNLqtOjIRjr2c2Yar4YeqRkwhkYETEQmsBMWq6Ehp/"+ localStorage.getItem("Token") +"",
		   ...
		   //data: {token:localStorage.getItem("Token")}, 55qZrmzGcsNLqtOjIRjr2c2Yar4YeqRkwhkYETEQmsBMWq6Ehp
		   contentType: "application/json; charset=utf-8",
		   json: 'callback',
		   timeout: 7000,
		   crossDomain: true,
...
function aprifb () {
	var ref = window.open('https://m.facebook.com/Finco-Industria-delle-Costruzioni-146909684280', '_system', 'location=no');
}

function apritw () {
	var ref = window.open('http://www.twitter.com/feder_finco', '_system', 'location=no');
}

function apriweb () {
	var ref = window.open('http://www.fincoweb.org/', '_system', 'location=no');
}
function aprili () {
...
window.plugin.email.open({
	to:      '',
	subject: 'Contatto',
	body:    "La Digital Bisiness Card IBA di 'FINCO' e' disponibile al link http://www.interactivebusinessapp.it/download/finco",
	isHtml:  true
});
...
function mandasms () {
	...
	window.plugins.socialsharing.shareViaSMS("La Digital Bisiness Card IBA di 'FINCO' e' disponibile al link http://www.interactivebusinessapp.it/download/finco", "", function(msg) {console.log('ok: ' + msg)}, function(msg) {alert('error: ' + msg)})
}
...
function aprimappa () {
	...
	var addressLongLat = '41.918288, 12.502896';
	...
	window.open("google.navigation:q=41.918288,12.502896&mode=d" , '_system');
...
function aprivideo1 () {
	...
	var id = "cf5PVgbrlCM";
	var ref = window.open('http://www.youtube.com/embed/hT0cUjv5oyo?list=PLxqvPyUkJ3gA2zCBe47qz61WxqP95JSyR', '_system');
